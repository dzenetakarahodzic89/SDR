...
public class ShazamIntegrationServiceTest extends BasicTestConfiguration {
	...
	public void testFindShazamIntegration() {
		try {
			...
			List<ShazamIntegrationResponse> shazamIntegrationFindResponse = shazamIntegrationService.find(filterRequest)
					.getPayload();
	...
	public void testCreateShazamIntegration() {
		try {
			...
			PayloadResponse<ShazamIntegrationResponse> ShazamIntegrationFindResponse = shazamIntegrationService
					.create(req);
			Assertions.assertThat(ShazamIntegrationFindResponse.getPayload()).as("Check all fields")
					.usingRecursiveComparison().ignoringFields("created", "createdBy", "modified", "modifiedBy")
					.isEqualTo(newShazamIntegration);
		} catch (Exception e) {
			Assert.fail();
		}
	}
	...
	public void testUpdateShazamIntegration() {
		try {
			...
			Mockito.when(shazamIntegrationRequestValidation.validateUpdateShazamIntegrationRequest(request))
					.thenReturn(null);
			Mockito.when(shazamIntegrationDAO.findByPK(request.getEntity().getId()))
					.thenReturn(shazamIntegrationEntity);
			...
			Assertions.assertThat(shazamIntegrationUpdateResponse.getPayload()).as("Check all fields")
					...
					.isEqualTo(shazamIntegrationResponse);
		} catch (Exception e) {
			Assert.fail();
		}
	}
	...
	public void testDeleteShazamIntegration() {
		try {
			...
			Assertions.assertThat(shazamIntegrationFindResponse.getPayload())
					.isEqualTo("Shazam Integration removed successfully!");
		} catch (Exception e) {
			Assert.fail();
		}
	}
