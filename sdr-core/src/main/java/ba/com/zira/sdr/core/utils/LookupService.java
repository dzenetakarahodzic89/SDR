...
public class LookupService {
	...
	public static String getUrl(final Long key, final Map<Long, List<CoverImageHelper>> lookup, final Random rand) {
		if (key != null) {
			if (rand != null) {
				return lookup.get(key) == null ? null
						: lookup.get(key).get(rand.nextInt(lookup.get(key).size())).getUrl();
			} else {
	...
	public <E> void lookupSongDateOfRelease(List<E> values, Function<E, Long> getter,
			BiConsumer<E, LocalDateTime> setter) {
	...
	public <E> void lookupMultiSearchCoverImage(final List<E> values, final Function<E, Long> getterForId,
			final Function<E, String> getterForType, final BiConsumer<E, String> setter,
			final Function<E, String> getterForImage) {
	...
	public <E> void handleDefaultImage(final E r, final BiConsumer<E, String> setter,
			final Function<E, String> getterForImage) {
	...
	public <E> void lookupObjectNamesByIdAndType(final String objectType, final List<E> values,
			final Function<E, Long> getter, final BiConsumer<E, String> setter) {
	...
	public <E> void lookupAudio(List<E> values, Function<E, Long> getter, BiConsumer<E, String> setter) {
		...
		if (!(ids == null || ids.isEmpty())) {
			Map<Long, List<CoverImageHelper>> lookup = mediaStoreDAO.getUrlsForList(ids, ObjectType.SONG.getValue(),
					"AUDIO");
	...
	public void lookupCoverImage(List<InstrumentEntity> asList, Object getter, String value, Object setter,
			Object getterForImage) {
