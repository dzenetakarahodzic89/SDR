...
public class BattleDAO extends AbstractDAO<BattleEntity, Long> {
    public BattleSingleResponse findLastBattleTurn(Long id) {
        var hql = "select new ba.com.zira.sdr.api.model.battle.BattleSingleResponse(sb.id,sb.name, sbt.turnNumber, sbt.mapState, sbt.teamState,sbt.turnCombatState) from BattleEntity sb join BattleTurnEntity sbt on sb.id =sbt.battle where sb.id = :id order by sbt.turnNumber desc";
