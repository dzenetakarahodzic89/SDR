...
public class ChordProgressionDAO extends AbstractDAO<ChordProgressionEntity, Long> {
    ...
    public List<ChordSongAlbumEraResponse> getAllChordProgressionSongNumberByEras() {
        var hql = "select distinct new ba.com.zira.sdr.api.model.chordprogression.ChordSongAlbumEraResponse(se.id,se.name,scp.id,ss.name) from ChordProgressionEntity scp join SongEntity ss on scp.id = ss.chordProgression.id join SongArtistEntity ssa on ssa.song.id =ss.id join AlbumEntity sa on ssa.album.id =sa.id join EraEntity se on sa.era.id =se.id";
